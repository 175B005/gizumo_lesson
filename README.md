...
**3月おわり！研修期間終わり！　本格的に会社入り！やったね。。**

**3/28(木)**
- 仮想デスクトップubuntu の　linux環境でちょっとやった。成果はほぼなし。。

      - 日本語対応と、外国キーボード慣れ？コマンド慣れかな？？
      
      - 情報もなんかあったが、書いている人間の程度が低い。。
      
      - 説明ではなく、メモ。。　その通りやってもできないっていうｗｗ
      
      - ちょっと自分で試して、何ができそうか調べてみたい。
...
- 説明と自分の修正をば。。
      ...
      - 微調整などしてたのと、jquery の１０のhtmlメソッドについて、解説、自分の修正をしていた。
      
      - html()は上書きでタグの要素を作る、つまり、指定する場所は最初から空で開けておく必要がある。
      
      - そして、削除をいちいちしなくても、勝手に上書きなので、emptyの処理が丸々いらないが、
      
      - そこに囚われていた。。一度前回の要素を保存しておき、新規のlist２０件をそこに追加html()で再度追加する流れ。

***

**3/15(木)**
- greenhorn Mac Vagrant 使ってみた。

      - mac が使いやすすぎる。。すげぇ。
      
      - 環境を整えておくことは重要ですね。。vagrant の仮想環境を作って、そこにデータベースを置く。
      
      - 前に自分でやっていた、仮想デスクトップからのwindowsのデータアクセスとは　完全に逆のこと。
      
      - パスを通しておくことがいかに重要かよいうことすね。
      
      - mac買いましょう。コマンドもwindowsで使える様にはしましたが、それでは足りんということがよくわかった。簡単すぎる。。
***

**3/14(木)**
- greenhorn の課題完全完了

      - 日本語への対応（DB側での、文字コード設定）ができていなかったので、対応。。
      
      - ついでに記事も書いといた。。初Qiita記事。。

[俺的コマンドまとめ　文字コード編](https://qiita.com/175B005/items/14e81acbe68939c49981)
...
- greenhorn 課題完了？
      ...
      - たぶん課題おわった。日報機能は難しくないが、その設定がむずい。
      
      - データ型の指定など、気を付ける点を次回まとめ、消せるところは消していく方向で。
...
- 昨日の時点でmigrateファイルとtableは用意しておいたので、あとはコード！

- 基本は取得する内容をかき出し、それに合った形にしてデータを渡したりする。。

- 作るものは大体決まっているので、そんなに大変ではない。。

- ダミ―データから、管理者機能の表示はできた。user側の表示と、追加もできてそうだが、comentsなどのデータがnullであるので、

- うまくいってないということで、しっかりと直しておく。バリデーションとかするのかな？？？とりまやっていく方向でｗｗ

***

**3/07(木)**
- greenhorn バグ修正　とりま完了

      - よく見てみると管理者のところに表示されてるのはnameが安藤さんだったけど、見たらadminの名前やん。
      
      - idが取得で来てなくて、id=1になってしまっていたので、userinfosでid1の人が表示されていた。。
      
      - 登録のしょりでは、その前にメールアドレスからユーザーに仮登録されているメールアドレスを使用して、idを特定
      
      - 反映させてみたらおｋだったのでよかった。たぶん処理フローは違うので、指摘してもらおう。。
      
      - と、思ったけどセキュリティ的には問題ないのでこれでOK.ただ、getでメールに追加する方法もあるということで、、、
      
      - 改修は完了。色々やり過ぎたらしい。。課題は日報を一から作る事。速攻で終わらせるで。今回は答えがあるんやｗｗｗぐへへ
      

***

**3/01(金)**
- greenhorn 管理者作成　登録できたけど、なんか違う。。

      - テーブルみても分からんし、どこに表示されていればいいのか？？
      
      - まず、adminは一覧に表示されてないので、表示されてなくていいんじゃね？？
      
      - 分からん、仕様がまったくわからん。。なにが正解なんや。。

***

**2/27(水)**
- greenhorn　管理者作成　登録画面

      - 登録画面にて、デコードをしている箇所があった。　エンコードをして、クエリーデータを送らなきゃ。。タイミング不明。
      
      - urlでgetでアクセスしたときに、なかった。－＞queryのデータが含まれるので、それを?後に追加して、データを送れるようにする。
      
      - そのため、データは変なことが起きないよう、エンコードしておく必要がある。
